...
import { PassportGoogleUser, Session, UserType } from '../../shared/types';
 ...
 * Note that user_id is autogenerated when creating user.
...
/**
 * Check express request.Session object to see if user is currently logged in
 * @param req express Request object
 * @param res express Response object
 */
...
const forgotPassword = async (req: Request, res: Response) => {
   try {
      ...
      if (!user) {
         ...
         const link = `${process.env.EMAIL_HOST}/passwordReset?token=${resetToken}&id=${user.user_id}`;
...
const resetPassword = async (req: Request, res: Response) => {
   ...
   try {
      const resetToken = (await tokensModel.findOne(
         body.userId
      )) as unknown as null | { token: string };
      if (!resetToken) {
